...
author: "[iPoolSeq-Pipeline](https://github.com/tgstoecker/ipoolseq-pipeline/tree/cbi_transposon) `r snakemake@params$version`"
...
essential <- read.table(snakemake@input$essential, header=TRUE)
...
The following table lists the main input files used to compute differential virulences, and the name of the generated results table. The results table lists, for each mutant strain, the change in virulence (as a log2 fold change of normalized output abundance relative to the neutral reference set), p-values, and FDR-corrected (using the method of Benjamini-Hochberg) q-values for the significance of the change.

```{r table_files}
knitr::kable(data.frame(row.names=c("list of mutants in (pre-infection) input pool",
                                    "list of mutants in (pre-infection) output pool",
                                    "5' mutant abundances in (pre-infection)  input pool",
                                    "3' mutant abundances in (pre-infection)  input pool",
                                    "5' mutant abundances in (post-infection) output pool",
                                    "3' mutant abundances in (post-infection) output pool",
                                    "differential virulence results table"),
                        file=c(make_link(basename(snakemake@input$isites_in)),
                               make_link(basename(snakemake@input$isites_out)),
...
local({
  ...
  stats_in <- data.frame(row.names=stats_in_5p$`After Step`,
  ...
  for(c in c('#5\' Reads Input', '#3\' Reads Input', '#5\' UMIs Input', '#3\' UMIs Input')) {
    stats_in[[c]] <- ifelse(!is.na(stats_in[[c]]),
                            paste0(stats_in[[c]], " (",
                                   round(100*stats_in[[c]]/(stats_in[[c]][min(which(!is.na(stats_in[[c]])))])), "%)"),
                            NA)
  }
  knitr::kable(stats_in)
})
```
...
local({
  ...
  stats_out <- data.frame(row.names=stats_out_5p$`After Step`,
  ...
  for(c in c('#5\' Reads Output', '#3\' Reads Output', '#5\' UMIs Output', '#3\' UMIs Output')) {
    stats_out[[c]] <- ifelse(!is.na(stats_out[[c]]),
                             paste0(stats_out[[c]], " (",
                                    round(100*stats_out[[c]]/(stats_out[[c]][min(which(!is.na(stats_out[[c]])))])), "%)"),
                             NA)
  }
  knitr::kable(stats_out)
})
```

## Correlation of 3' and 5' Flank Abundances

Since the abundance of each mutant strain is measured twice, once for the 5' and once for the 3' flank of the cassette integration, the correlation of these two measurements provides a quality check of the data. The following table lists the correlation of both the raw, and the TRUmiCount-corrected UMI counts detected for the 5' and 3' flanks of each mutant strain. Correlations are computed from the *square roots* of counts respectively abundances (to avoid overdue influence of large counts), and with the intercept forced to zero (regression line passing through the origin).
...
local({
  p <- function(n.obs.5p, n.obs.3p, labels, type, first=FALSE, main) {
    ...
    if (any(!missing))
      o <- o %>% add_markers(x=n.obs.5p[!missing],
                             y=n.obs.3p[!missing],
                             color=I("black"),
                             text=labels[!missing],
                             name="UMIs found for 5' and 3' flank",
                             legendgroup="nz",
                             marker=list(symbol="circle-open"))
    if (any(missing))
      o <- o %>% add_markers(x=pmax(n.obs.5p[missing], 1.0, na.rm=TRUE),
                             y=pmax(n.obs.3p[missing], 1.0, na.rm=TRUE),
                             color=I("black"),
                             text=labels[missing],
                             name="No UMIs found for 5' or 3' flank",
                             legendgroup="z",
                             marker=list(symbol="x"))
    o <- o %>% layout(xaxis=list(type="log", title=paste0(type, " #UMIs 5'")),
  ...
  p.raw.in <- join.flanks.as.columns(counts.flank.in)[, {
    p(n5p, n3p, mutant, main="Input 5' vs. 3' flank (raw)", type="Raw", first=TRUE)
  }]
  p.raw.out <- join.flanks.as.columns(counts.flank.out)[, {
    p(n5p, n3p, mutant, main="Output 5' vs. 3' flank (raw)", type="Raw")
  }]
  p.corr.in <- d <- join.flanks.as.columns(counts.flank.in)[, {
    p(n5p/(1-l5p), n3p/(1-l3p), mutant, main="Input 5' vs. 3' flank (loss-corr.)", type="Loss-corr.")
  }]
  p.corr.out <- d <- join.flanks.as.columns(counts.flank.out)[, {
    p(n5p/(1-l5p), n3p/(1-l3p), mutant, main="Output 5' vs. 3' flank (loss-corr.)", type="Loss-corr.")
  }]
...
In iPool-Seq-based screens, the abundances of the different mutant strains are often spread over multiple orders of magnitude, and differences in input abundances can thus affect the abundance in the output more strongly than the mutant's phenotypes. The statistical model used to detect significant changes of virulence must thus take the input abundance into account, and assumes that for neutral mutant strains, input and output abundances are proportional.
...
local({
  ...
  p.raw <- counts[, {
    p(n.in, n.out, mutant, main="Input vs. Output (raw)", type="Raw", first=TRUE)
  }]
  p.corr <- counts[, {
    p(abundance.in, abundance.out, mutant, main="Input vs. Output (loss-corr.)", type="Loss-corr.")
  }]
...
## Overlap of input and output pools

Virulences and p-values are computed only for mutants which are detected in the input *and* the output pool. The following Venn diagramm shows the number of mutants detected in each pool and their overlap.

```{r plot_io_overlap, fig.height=1.5, fig.width=3}
library(VennDiagram)
grid.newpage()
local({
  d <- draw.pairwise.venn(counts[!is.na(abundance.in) & (abundance.in > 0), .N],
                          counts[!is.na(abundance.out) & (abundance.out > 0), .N],
                          counts[!is.na(abundance.out) & !is.na(abundance.out) & (abundance.in > 0) & (abundance.out > 0), .N],
                          category=c("input", "output"),
                          cat.dist=c(-0.025, -0.025), cat.pos=c(0,0))
})
```


## Abundance distribution for mutant classes *intergenic*, *non-essential* and *essential*

The following boxplots show the distribution of abundances for mutants affecting either an *intergenic region*, an *non-essential gene* or an *essential gene*.

```{r plot_mutantclass_abundance}
local({
  class.levels = c("intergenic", "non-essential", "essential")
  data <- counts[, list(
    class=factor(ifelse(is.na(gene), "intergenic",
                       ifelse(genes[pmax(gene, 1, na.rm=TRUE)]$Essential,
                              "essential", "non-essential")),
                levels=class.levels, ordered=TRUE),
    mutant,
    abundance.in,
    abundance.out
  )]
  p.in <- plot_ly(data, x=~class, y=~abundance.in, color=I("black"),
                  text=~mutant, type="box", showlegend=FALSE) %>%
    layout(xaxis=list(title="gene class"),
           yaxis=list(title="Abundance (Input)"))
  p.out <- plot_ly(data, x=~class, y=~abundance.out, color=I("black"),
                   text=~mutant, type="box", showlegend=FALSE) %>%
    layout(xaxis=list(title="gene class"),
           yaxis=list(title="Abundance (Output)"))
  subplot(list(p.in, p.out), nrows=1,
          titleX=TRUE, titleY=TRUE, margin=0.12) %>%
  toWebGL()
})
```
...
The (raw) output UMI count $N_\text{out}$ of a neutral mutant strain given its (raw) input UMI count $N_\text{in}$ and TRUmiCount-estimated loss (i.e. fraction of lost UMIs) $\ell_\text{in}$ for the input and $\ell_\text{out}$ for the output is modelled with the following negative binomial model
...
The optimal (likelihood-maximazing) values for parameters $\lambda$ and $d$ for the reference set of neutral mutant strains are:
...
For every mutant strain detected in both input and output with (raw) UMI counts $N_\text{in}$ respectively $N_\text{out}$ (counts of 5' and 3' flanks added) and TRUmiCount-estimated  losses (i.e. fraction of lost UMIs) $\ell_\text{in}$ and $\ell_\text{out}$ (losses for 5' and 3' flanks averaged), the virulence log2 fold-change compared to the neutral reference set is
...
Due to the negative binomial model (see *Statistical Model and Parameters*), the power to detect changes in virulence increases with the input abundance of a mutant strain. The following plot shows the change in virulence (as log2 fold-change) against the input abundance, and indicates which mutant strins have a virulence significantly different from the neutral reference set at a *false discovery rate* (FDR) threshold of `r PLOT.FDR.THRESHOLD`.
...
local({
  ...
  a.in <- 10**seq(from=log10(0.5), to=2*log10(max(results$abundance.in, na.rm=TRUE)), length.out=200)
  ...
  f.neutral <- expression(is.neutral & !is.na(low.qval) & !is.na(high.qval))
  f.insig <- expression(!is.neutral & (low.qval >= PLOT.FDR.THRESHOLD) & (high.qval >= PLOT.FDR.THRESHOLD))
  f.sig.low <- expression(low.qval <= PLOT.FDR.THRESHOLD)
  f.sig.high <- expression(high.qval <= PLOT.FDR.THRESHOLD)
  ...
  add_trace(x=results[eval(f.insig), abundance.in],
            y=results[eval(f.insig), log2fc],
            text=results[eval(f.insig), mutant],
            name='insignificant change of virulence',
            marker=list(symbol="circle-open", color='darkgrey'),
            type="scatter", mode="markers") %>%
  add_trace(x=results[eval(f.neutral), abundance.in],
            y=results[eval(f.neutral), log2fc],
            text=results[eval(f.neutral), mutant],
            name='neutral reference set',
            marker=list(symbol="circle-open", color='darkgreen'),
            type="scatter", mode="markers") %>%
  add_trace(x=results[eval(f.sig.low), abundance.in],
            y=results[eval(f.sig.low), log2fc],
            text=results[eval(f.sig.low), mutant],
            name='significantly reduced virulence',
            marker=list(symbol="circle-open", color='red'),
            type="scatter", mode="markers") %>%
  add_trace(x=results[eval(f.sig.high), abundance.in],
            y=results[eval(f.sig.high), log2fc],
            text=results[eval(f.sig.high), mutant],
            name='significantly increased virulence',
            marker=list(symbol="circle-open", color='blue'),
            type="scatter", mode="markers") %>%
  layout(xaxis=list(range=c(log10(0.5), ceiling(max(log10(results$abundance.in), na.rm=TRUE))), type="log",
...
__________
(The grey area of insignificance virulence changes is only approximate -- it does not take any *false discovery rate* (FDR) or *multiple testing* correction into account, and it is computed from an average *loss*, instead of the per-mutant loss percentage computed by [TRUmiCount](http://www.cibiv.at/software/trumicount). Mutant strains are called significant or insignificant based on a FDR threshold of `r PLOT.FDR.THRESHOLD`)
...
DT::datatable(results[!is.na(log2fc), list(
    mutant, is.neutral, gene_name, log2fc, pval=low.pval, qval=low.qval, sig=sig.stars(low.qval)
  )],
...
__________
(*log2fc* contains the log2 fold-changes of the virulence compared to the neutral reference set, *pval* contains p-values for the significance of *log2fc* < 0 under a negative binomial model, *qval* contains FDR-corrected p-values, in *sig* \*\*\*\*\* means *qval* <= 0.00001, \*\*\*\* means *qval* <= 0.0001, \*\*\* means *qval* <= 0.001, \*\* means *qval* <= 0.01, \* means *qval* <= 0.05, + means *qval* <= 0.1. By default only mutant strains with *qval* <= 0.1 are shown, remove the column filter to show all)
...
```{r table_enriched}
DT::datatable(results[!is.na(log2fc), list(
  mutant, is.neutral, gene_name, log2fc, pval=high.pval, qval=high.qval, sig=sig.stars(high.qval)
  )],
  filter="top", rownames=FALSE,

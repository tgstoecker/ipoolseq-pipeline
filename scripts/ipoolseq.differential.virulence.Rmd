...
The following table lists the main input files used to compute differential virulences, and the name of the generated output table. The output table lists, for each knockout, the change in virulence (as a log2 fold change of normalized output abundance relative to the neutral reference set), and p-values (plus FDR-corrected q-values) for the significance of the change.
...
knitr::kable(data.frame(row.names=c("knockouts",
                                    ...
                                    "virulence log2 fold changes & significance"),
...
The number of remaining read pairs and the number of unique UMIs within these pairs after each analysis step. The percentages are relatie to the *first* number within each column. Since unique UMIs are determined for the 5' and 3' flank of each knockout, no UMI counts are reported until reads have been assigned to the individual knockouts (and their flanks).

Also check the *TRUmiCount report* (found above unter *1. Files*) for details about the performance of the TRUmiCount UMI filtering step.
...
In iPool-Seq-based screens, the abundances of the different knockout strains are often spread over multiple orders of magnitude, and differences in input abundances can thus affect the abundance in the output more strongly than effects of the knockout on the strain's virulence. The statistical model used to detect significant changes of virulence corrects for differences in input abundance by assuming that for neutral knockouts, input and output abundances are proportional.

The following table shows the observed correlation of (loss-corrected by TRUmiCount) input and output abundances (averaged across the 5' and 3' flank measurements)
...
The (raw) output abundance (i.e. UMI count) $N_\text{out}$ of a neutral knockout given its (raw) input abundance $N_\text{in}$ and TRUmiCount-estimated loss $\ell_\text{in}$ in the input and $\ell_\text{out}$ in the output is modelled with the following negative binomial model
...
knitr::kable(signif(null.model, digits=3),
             col.names=c('Relative output library size (lambda)', 'Squared CV of biological noise (d)'))
```

Parameter $\lambda$ measurs the relative size of the output library (in loss-corrected total number of UMIs) compared to the input. Parameter $d$ represents squared coefficient of variation of the output abundances due to *biological noise*, i.e. due to differences in the growth of neutral mutants.

# 3. Results

For every knockout strain detected in both input and output with (raw) UMI counts $N_\text{in}$ respectively $N_\text{out}$ (counts of 5' and 3' flanks added) and TRUmiCount-estimated  losses $\ell_\text{in}$ and $\ell_\text{out}$ (losses for 5' and 3' flanks averaged), the virulence log2 fold-change compared to the neutral reference set is

$$
\log_2 \delta v = \log_2 \frac{N_\text{out} / (1-\ell_\text{out})}{N_\text{in} / (1-\ell_\text{in})}.
$$
...
__________
(*log2fc* contains the log2 fold-changes of the virulence compared to the neutral reference set, *pval* contains p-values for the significance of *log2fc* < 0 under a negative binomial model, *qval* contains FDR-corrected p-values, in *sig* \*\*\*\*\* means *qval* <= 0.00001, \*\*\*\* means *qval* <= 0.0001, \*\*\* means *qval* <= 0.001, \*\* means *qval* <= 0.01, \* means *qval* <= 0.05, + means *qval* <= 0.1)

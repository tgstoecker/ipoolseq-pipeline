...
snakemake <- new("SnakemakeStub",
                 input=list(isites_in=paste("..", "tests", "data", design, paste0(replicate, "-in.isites.gff3.gz"), sep="/"),
                            isites_out=paste("..", "tests", "data", design, paste0(replicate, "-out.isites.gff3.gz"), sep="/"),
                            pool_in_5p=paste("..", "tests", "data", design, paste0(replicate, "-in:5p.count.tab"), sep="/"),
                            pool_in_3p=paste("..", "tests", "data", design, paste0(replicate, "-in:3p.count.tab"), sep="/"),
                            pool_out_5p=paste("..", "tests", "data", design, paste0(replicate, "-out:5p.count.tab"), sep="/"),
                            pool_out_3p=paste("..", "tests", "data", design, paste0(replicate, "-out:3p.count.tab"), sep="/"),
                            stats_in_5p=paste("..", "tests", "data", design, paste0(replicate, "-in:5p.stats.tab"), sep="/"),
                            stats_in_3p=paste("..", "tests", "data", design, paste0(replicate, "-in:3p.stats.tab"), sep="/"),
                            stats_out_5p=paste("..", "tests", "data", design, paste0(replicate, "-out:5p.stats.tab"), sep="/"),
                            stats_out_3p=paste("..", "tests", "data", design, paste0(replicate, "-out:3p.stats.tab"), sep="/"),
                            trumicount_pdf_in_5p=paste("..", "tests", "data", design, paste0(replicate, "-in:5p.count.pdf"), sep="/"),
                            trumicount_pdf_in_3p=paste("..", "tests", "data", design, paste0(replicate, "-in:3p.count.pdf"), sep="/"),
                            trumicount_pdf_out_5p=paste("..", "tests", "data", design, paste0(replicate, "-out:5p.count.pdf"), sep="/"),
                            trumicount_pdf_out_3p=paste("..", "tests", "data", design, paste0(replicate, "-out:3p.count.pdf"), sep="/"),
                            fastqc_html_in_5p_r1=paste("..", "tests", "data", design, paste0(replicate, "-in:5p.fastqc.1.html"), sep="/"),
                            fastqc_html_in_5p_r2=paste("..", "tests", "data", design, paste0(replicate, "-in:5p.fastqc.2.html"), sep="/"),
                            fastqc_html_in_3p_r1=paste("..", "tests", "data", design, paste0(replicate, "-in:3p.fastqc.1.html"), sep="/"),
                            fastqc_html_in_3p_r2=paste("..", "tests", "data", design, paste0(replicate, "-in:3p.fastqc.2.html"), sep="/"),
                            fastqc_html_out_5p_r1=paste("..", "tests", "data", design, paste0(replicate, "-out:5p.fastqc.1.html"), sep="/"),
                            fastqc_html_out_5p_r2=paste("..", "tests", "data", design, paste0(replicate, "-out:5p.fastqc.2.html"), sep="/"),
                            fastqc_html_out_3p_r1=paste("..", "tests", "data", design, paste0(replicate, "-out:3p.fastqc.1.html"), sep="/"),
                            fastqc_html_out_3p_r2=paste("..", "tests", "data", design, paste0(replicate, "-out:3p.fastqc.2.html"), sep="/")),
...
knockouts <- readGFF(snakemake@input$isites_in, version=3)
# TODO: Find neutral knockouts!
NEUTRAL <- NULL
...
load.trumicount.output <- function(file, flank) {
  ...
  # Make sure there is exactly one row per knockout
  allgenes <- data.table(gene=knockouts$Name)[,, on="gene"]
  # The raw "gene" column contains <gene>:<flank>, split into two separate fields
  t[, knockout := gene ]
  t[, knockout.chr := as.factor(sub(as.character("^(.*):(\\d+)\\([+-]\\)$"), "\\\\1", gene)) ]
  t[, knockout.pos := as.numeric(sub(as.character("^(.*):(\\d+)\\([+-]\\)$"), "\\\\2", gene)) ]
  t[, knockout.dir := as.factor(sub(as.character("^(.*):(\\d+)\\([+-]\\)$"), "\\\\3", gene)) ]
  t[, flank := flank ]
}
...
p <- function(abd.in, abd.out, labels, type, first=FALSE, main) {
  ...
  o <- o %>% add_markers(x=abd.in[!missing],
                         ...
                         marker=list(symbol="circle-open"))
  if (any(missing))
    o <- o %>% add_markers(x=pmax(abd.in[missing], 1, na.rm=TRUE),

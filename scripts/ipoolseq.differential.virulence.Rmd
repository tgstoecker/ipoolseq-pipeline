...
counts <- merge(counts.in, counts.out, by="mutant", suffixes=c(".in", ".out"))[,list(
  mutant,
  ...
  abundance.in=ifelse(n.in > 0, n.in / (pmax(flanks.in, flanks.out, na.rm=TRUE) * (1-loss.in)), 0),
  abundance.out=ifelse(n.out > 0, n.out / (pmax(flanks.in, flanks.out, na.rm=TRUE) * (1-loss.out)), 0)
)]

# Fill in index of the gene (if any) affected by a mutant
local({
  allKOmeta <- data.table(mutant=mutants$Name, gene=mutants$GeneIndex)
  counts[, "gene" := allKOmeta[counts, gene, on=c("mutant")] ]
})
```
...
add_trace(x=results[!is.neutral & (low.qval >= PLOT.FDR.THRESHOLD) & (high.qval >= PLOT.FDR.THRESHOLD), abundance.in],
          ...
          text=results[!is.neutral & (low.qval >= PLOT.FDR.THRESHOLD) & (high.qval >= PLOT.FDR.THRESHOLD), mutant],
...
add_trace(x=results[is.neutral==TRUE, abundance.in],
          ...
          text=results[is.neutral==TRUE, mutant],
...
add_trace(x=results[is.neutral==FALSE & !is.finite(low.qval) & !is.finite(high.qval), abundance.in],
          ...
          text=results[is.neutral==FALSE & !is.finite(low.qval) & !is.finite(high.qval), mutant],
          name='non-neutral',
...
add_trace(x=results[low.qval <= PLOT.FDR.THRESHOLD, abundance.in],
          ...
          text=results[low.qval <= PLOT.FDR.THRESHOLD, mutant],
...
add_trace(x=results[high.qval <= PLOT.FDR.THRESHOLD, abundance.in],
          ...
          text=results[high.qval <= PLOT.FDR.THRESHOLD, mutant],

...
(UMI counts of 5' and 3' flanks are added to obtain $N_\text{in}$ and $N_\text{out}$, losses for 5' and 3' flanks are averaged to obtain $\ell_\text{in}$ and $\ell_\text{out}$). The optimal (likelihood-maximazing) values for parameters $\lambda$ and $d$ for the reference set of neutral knockouts are:
...
Parameter $\lambda$ measurs the relative size of the output library (i.e. loss-corrected total number of UMIs per neutral knockout) compared to the input. Parameter $d$ represents squared coefficient of variation of the output abundances due to *biological noise*, i.e. due to differences in the growth of neutral mutants.

...
local({
  ...
  knockouts$GeneID <<- NA_character_
  knockouts$GeneID[queryHits(hits)] <<- genes[subjectHits(hits)]$ID
  knockouts$GeneName <<- NA_character_
  knockouts$GeneName[queryHits(hits)] <<- genes[subjectHits(hits)]$Name
})
# Mark knockouts not assigned to genes as neutral
knockouts$Neutral <- ifelse(is.na(knockouts$GeneID), 1, 0)
...
# Fill in gene_name and gene_id columns
local({
  allKOmeta <- data.table(knockout=knockouts$Name, gene_id=knockouts$GeneID, gene_name=knockouts$GeneName)
  counts[, c("gene_id", "gene_name") := allKOmeta[counts, list(x.gene_id, x.gene_name), on=c("knockout")] ]
})
```
...
results <- counts[, list(
  knockout, is.neutral, gene_id, gene_name,
